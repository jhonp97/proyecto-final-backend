...
export const loginUser = async (req, res, next) => {
  try {
    ...
    const existingUser = await User.findOne({ email }).populate("solicitudAmistad", "username fotoPerfil");
    if (!existingUser) {
...
export const getCurrentUser = async (req, res, next) => {
  try {
    ...
    const user = await User.findById(userId).select("-password").populate("solicitudAmistad", "username fotoPerfil");
    if (!user) {
